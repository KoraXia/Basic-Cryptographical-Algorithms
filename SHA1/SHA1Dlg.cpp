...
static char Message_Digest[20];
extern char * FromHexString(char *input);
extern char * ToHexString(char * input);
extern void SHA1PadMessage(SHA1Context *context);
...
void CSHA1Dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT2, m_abstract);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CSHA1Dlg, CDialog)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON1, OnHash)
	ON_BN_CLICKED(IDC_BUTTON2, OnClean)
	//}}AFX_MSG_MAP
	ON_EN_CHANGE(IDC_EDIT1, &CSHA1Dlg::OnEnChangeEdit1)
END_MESSAGE_MAP()
...
void CSHA1Dlg::OnHash() 
{
	...
	memcpy(inputstr, m_plain, strlen(m_plain));//明文


    //补充代码
	SHA1Reset(&sha);
	SHA1Input(&sha, inputstr, strlen(m_plain));
	SHA1PadMessage (&sha);
    //输出结果。注意！！实验报告中要求对以下的for语句完成的功能进行解释。
	for (i = 0; i < 20; i++) {
         Message_Digest[i] = (BYTE)((sha.Message_Digest[i >> 2] >> ((3 - (i & 3)) * 8)) &255);
    }
	//修改代码,实现十六进制格式的显示

	m_abstract = ToHexString(Message_Digest);
	
	GetDlgItem(IDC_EDIT2)->SetWindowText(m_abstract);
    
}

void CSHA1Dlg::OnClean() 
{
	// TODO: Add your control notification handler code here
