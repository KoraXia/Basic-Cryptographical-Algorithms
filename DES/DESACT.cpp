...
// 扩展置换表
const static char Extension_Table[48] = {
...
void RotateL(bool *In, int len, int loop)
{
    ...
    //补充程序
	//提示memcpy(Tmp, In, loop); tmp在DES.h中已定义。
}
...
void F_func(bool In[32], const bool Ki[48])
{
    ...
    static bool MR[48];
    Transform(MR, In, Extension_Table, 48);// 变换
    Xor(MR, Ki, 48);// 异或
    S_func(In, MR);// S 盒代替
...
// DES加解密函数
void DES_OneAct(char *Out, char *In, long datalen, const char *Key, int keylen, bool Type)
{
	SetKey(Key, keylen);
